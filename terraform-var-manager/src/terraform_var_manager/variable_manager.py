        ...
        while i < len(lines):
            ...
            if mline or value.strip() == "begin":
                ...
                while i < len(lines):
                    ...
                    # Check if this line is ONLY 'end' or 'end #...' (not '} end')
                    # This means 'end' should be at the start of the stripped line
                    if stripped == "end" or (stripped.startswith("end ") and "#" in stripped):
                        # Parse tags from the end line comment
                        if "#" in stripped:
                            _, *end_comment = stripped.split("#")
                            end_line_comment = end_comment[0] if end_comment else ""
                        i += 1
                        break
                ...
                if end_line_comment:
                    tags = [t.strip() for t in end_line_comment.split(",")]
                    for tag in tags:
                        if tag == "sensitive":
                            sensitive = True
                        elif tag == "hcl":
